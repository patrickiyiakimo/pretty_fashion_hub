...
// Note: The proxyRequest utility is now handled by the API routes
// This client component will fetch from those API routes which will use the proxy
...
export default function AdminDashboard() {
  ...
  async function fetchRecentActivity() {
    ...
    try {
      // Fetch data from multiple endpoints using the authenticated fetch helper
      const [partnersRes, consultationsRes, usersRes, logisticsRes] = await Promise.allSettled([
        authenticatedFetch("/api/partners?limit=5&status=pending"),
        authenticatedFetch("/api/consultations?limit=5&status=pending"),
        authenticatedFetch("/api/auth/admin/users?limit=5"),
        authenticatedFetch("/api/logistics?limit=5")
      ]);
  ...
  async function fetchDashboardStats() {
    ...
    try {
      const [partnersRes, consultationsRes, usersRes] = await Promise.allSettled([
        authenticatedFetch("/api/partners?status=pending"),
        authenticatedFetch("/api/consultations?status=pending"),
        authenticatedFetch("/api/auth/admin/users?limit=1")
      ]);
