...
export default function PartnersManagement() {
  ...
  }, [filters.status, filters.page, filters.search, authChecked]);
  ...
  async function loadPartners() {
    ...
    try {
      ...
      const url = `/api/partners${queryParams.toString() ? `?${queryParams.toString()}` : ''}`;
      ...
      if (!res.ok) {
        throw new Error(`Failed to load partners: ${res.status} ${res.statusText}`);
      }
      ...
      } else if (data.success && Array.isArray(data.partners)) {
  ...
  async function loadStats() {
    try {
      ...
      if (res.status === 401) {
        return; // Don't redirect here, next request will handle it
      }
      ...
      if (!res.ok) {
        ...
        setStats({
          total: partners.length,
          approved: partners.filter(p => p.status?.toLowerCase() === 'approved').length,
          pending: partners.filter(p => p.status?.toLowerCase() === 'pending').length,
          rejected: partners.filter(p => p.status?.toLowerCase() === 'rejected').length,
          suspended: partners.filter(p => p.status?.toLowerCase() === 'suspended').length,
          availableSlots: Math.max(0, 50 - partners.filter(p => p.status?.toLowerCase() === 'approved').length)
        });
        return;
      }
      ...
      if (data.stats) {
        ...
        // Calculate from partners if no stats endpoint
        statsData = {
          total: partners.length,
          approved: partners.filter(p => p.status?.toLowerCase() === 'approved').length,
          pending: partners.filter(p => p.status?.toLowerCase() === 'pending').length,
  ...
  async function updatePartnerStatus(partnerId, status, reason = "") {
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(errorData.message || errorData.error || `Failed to update status: ${res.status}`);
        return;
      }
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-600 text-sm font-medium">Approved</p>
                <p className="text-2xl font-bold text-green-600">
                  {stats?.approved || partners.filter(p => p.status?.toLowerCase() === 'approved').length}
                </p>
              </div>
              <HiCheckCircle className="w-8 h-8 text-green-600" />
            </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-600 text-sm font-medium">Pending</p>
                <p className="text-2xl font-bold text-yellow-600">
                  {stats?.pending || partners.filter(p => p.status?.toLowerCase() === 'pending').length}
                </p>
              </div>
              <HiClock className="w-8 h-8 text-yellow-600" />
            </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-600 text-sm font-medium">Rejected</p>
                <p className="text-2xl font-bold text-red-600">
                  {stats?.rejected || partners.filter(p => p.status?.toLowerCase() === 'rejected').length}
                </p>
              </div>
              <HiXCircle className="w-8 h-8 text-red-600" />
            </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">
                  {stats?.availableSlots || Math.max(0, 50 - partners.filter(p => p.status?.toLowerCase() === 'approved').length)}
                </p>
              </div>
                  ...
                  {partners.map((partner) => (
                    <tr key={partner._id || partner.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4">
                        <div>
                          <div className="font-medium text-gray-900">
                            {partner.businessName || partner.name || 'N/A'}
                          </div>
                          <div className="text-sm text-gray-500">
                            {partner.businessType || partner.type || 'N/A'}
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-900">
                          {partner.address?.city || partner.city || 'N/A'}, {partner.address?.state || partner.state || 'N/A'}
                        </div>
                        <div className="text-sm text-gray-500">
                          {partner.address?.country || partner.country || 'N/A'}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <StatusBadge status={partner.status} />
                      ...
                      <td className="px-6 py-4">
                        <div className="flex gap-2">
                          ...
                          {(partner.status?.toLowerCase() === 'pending' || !partner.status) && (
                            <>
                              <button
                                onClick={() => {
                                  setSelectedPartner(partner);
                          ...
                          {partner.status?.toLowerCase() === 'approved' && (
                            <>
                              <button
                                onClick={() => {
                                  setSelectedPartner(partner);
                          ...
                          {partner.status?.toLowerCase() === 'suspended' && (
                            <button
                              onClick={() => {
                                setSelectedPartner(partner);
                                setShowStatusModal('approved');
        ...
        <AnimatePresence>
          {showDetails && selectedPartner && (
            <PartnerDetailsModal
              ...
              onStatusUpdate={(status, reason) => {
                updatePartnerStatus(selectedPartner._id || selectedPartner.id, status, reason);
              }}
            />
          )}
        </AnimatePresence>
        ...
        <AnimatePresence>
          {showStatusModal && selectedPartner && (
            <StatusUpdateModal
              ...
              onConfirm={(reason) => {
                updatePartnerStatus(selectedPartner._id || selectedPartner.id, showStatusModal, reason);
              }}
            />
          )}
        </AnimatePresence>
    ...
    >
      ...
      >
        <div className="p-6 border-b border-gray-200">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-semibold text-gray-900">
              Partner Details - {partner.businessName || partner.name}
            </h2>
            <button
              onClick={onClose}
              className="text-gray-400 hover:text-gray-600 transition-colors"
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700">Business Name</label>
                <p className="mt-1 text-sm text-gray-900">{partner.businessName || partner.name || 'N/A'}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Business Type</label>
                <p className="mt-1 text-sm text-gray-900">{partner.businessType || partner.type || 'N/A'}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Tax ID / RC Number</label>
                <p className="mt-1 text-sm text-gray-900">{partner.taxId || partner.rcNumber || 'Not provided'}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Years in Business</label>
                <p className="mt-1 text-sm text-gray-900">{partner.yearsInBusiness || partner.years || 'Not specified'}</p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700">Email</label>
                <p className="mt-1 text-sm text-gray-900">{partner.email || 'N/A'}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Phone</label>
                <p className="mt-1 text-sm text-gray-900">{partner.phone || 'N/A'}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Website</label>
                <p className="mt-1 text-sm text-gray-900">{partner.website || 'Not provided'}</p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700">Street</label>
                <p className="mt-1 text-sm text-gray-900">{partner.address?.street || partner.street || 'N/A'}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">City</label>
                <p className="mt-1 text-sm text-gray-900">{partner.address?.city || partner.city || 'N/A'}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">State</label>
                <p className="mt-1 text-sm text-gray-900">{partner.address?.state || partner.state || 'N/A'}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Country</label>
                <p className="mt-1 text-sm text-gray-900">{partner.address?.country || partner.country || 'N/A'}</p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900 bg-gray-50 rounded-lg p-4">
                  {partner.description || partner.businessDescription || 'No description provided'}
                </p>
              </div>
              ...
              {partner.services && (
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-900">{partner.services}</p>
                </div>
              )}
              ...
              {partner.specialties && (
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-900">{partner.specialties}</p>
                </div>
              )}
            </div>
          </div>
...
function StatusUpdateModal({ partner, status, onClose, onConfirm }) {
  ...
  const statusMessages = {
    approved: {
      title: "Approve Partner",
      message: `Approve ${partner.businessName || partner.name} as a partner?`,
      button: "Approve Partner",
      color: "green"
    },
    rejected: {
      title: "Reject Application",
      message: `Reject ${partner.businessName || partner.name}'s partner application?`,
      button: "Reject Application",
      color: "red"
    },
    suspended: {
      title: "Suspend Partner",
      message: `Suspend ${partner.businessName || partner.name}'s partner account?`,
      button: "Suspend Partner",
      color: "gray"
    }
  };
      ...
      >
        ...
        <div className="p-6 border-t border-gray-200 flex gap-3">
          <button
            ...
            } text-white px-6 py-3 rounded-lg font-semibold transition-colors flex-1`}
          >
            {config.button}
          </button>
          <button
