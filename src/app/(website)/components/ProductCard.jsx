...
export default function ProductCard({ product }) {
  ...
  // ✅ UPDATED: IMAGE URL FUNCTION - USES RELATIVE PATHS
  const getImageUrl = (imagePath) => {
    if (!imagePath) {
      return "/api/placeholder-image";
    }

    // If it's already a proxy URL (but not a full URL)
    ...
    // If it's a full URL from Render
    if (imagePath.startsWith('http')) {
      try {
        const url = new URL(imagePath);
    ...
    // ✅ Return relative path through proxy
    return `/api/proxy-image/${filename}`;
  };
